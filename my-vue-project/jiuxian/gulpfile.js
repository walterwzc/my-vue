...
gulp.task('copypages', function () {
    gulp.src('./src/pages/**/*')
        .pipe(gulpif(option.env !== 'production', gulp.dest('./dev/pages')))
        .pipe(gulpif(option.env === 'production', gulp.dest('./dist/pages')))
})
...
if (option.env !== 'production') {
    gulp.task('default', ['copyhtml', 'copypages', 'copymock', 'copystatic', 'copylibs', 'sass', 'packjs', 'webserver', 'watch'], function () {
        console.log('done.')
    })
} else {
